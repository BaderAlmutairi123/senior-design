...
import { createOrgScenario } from "@/lib/organizations/org-scenario-actions";
...
export default function OrgScenarioForm({ orgId }: { orgId: string }) {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await createOrgScenario(orgId, formData);
      if (result.ok) {
  ...
  return (
    <form action={handleSubmit} className="max-w-3xl space-y-8">
      {error && (
        <div className="rounded-md border border-[#ff716c]/30 bg-[#ff716c]/10 px-4 py-3 text-sm text-[#ff716c]">
          {error}
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex items-center gap-2 rounded-md bg-[var(--cd-tertiary)] px-6 py-3 text-sm font-bold uppercase tracking-wider text-[var(--cd-surface)] transition-all hover:brightness-110 disabled:opacity-50"
        >
